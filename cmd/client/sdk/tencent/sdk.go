...
func (t *SDK) auth(provider *models.CloudProvider) error {
	credential := common.NewCredential(
		provider.CloudAuth.AccessKey,
		provider.CloudAuth.AccessSecret,
	)
...
func (t *SDK) billing(provider *models.CloudProvider) (float64, error) {
	credential := common.NewCredential(
		provider.CloudAuth.AccessKey,
		provider.CloudAuth.AccessSecret,
	)
...
func (t *SDK) deploy(provider *models.CloudProvider, tun *models.Tunnel) (string, error) {
	credential := common.NewCredential(
		provider.CloudAuth.AccessKey,
		provider.CloudAuth.AccessSecret,
	)
	...
	// 创建 SFC 客户端
	client, err := scf.NewClient(credential, *provider.Region, cpf)
	...
	// 腾讯云没有 cpu 大小设置选项
	request.MemorySize = common.Int64Ptr(int64(tun.TunnelConfig.Memory))
	// 需要记得打开 ws 支持
	...
	request.Code = &scf.Code{
		ImageConfig: &scf.ImageConfig{
			...
			ImageUri:  common.StringPtr("hkccr.ccs.tencentyun.com/seamoon/seamoon:" + consts.Version),
			Command:   common.StringPtr("/app/seamoon"),
			Args:      common.StringPtr("server -p " + *tun.Port + " -t " + string(*tun.Type)),
			ImagePort: common.Int64Ptr(int64(port)),
		},
	}
	...
	request.InstanceConcurrencyConfig = &scf.InstanceConcurrencyConfig{
		...
		MaxConcurrency: common.Uint64Ptr(uint64(tun.TunnelConfig.Instance)),
	}
	...
	config, err := json.Marshal(&triggerDesc{
		Api: triggerDescApi{
			AuthRequired: func() string {
				if tun.TunnelConfig.TunnelAuthType == types.Paris {
					return "TRUE"
				}
				return "FALSE"
			}(),
...
func (t *SDK) sync(provider *models.CloudProvider) ([]fcInfo, error) {
	credential := common.NewCredential(
		provider.CloudAuth.AccessKey,
		provider.CloudAuth.AccessSecret,
	)
